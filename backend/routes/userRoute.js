...
const { updateUser, registerUser, loginUser, logoutUser, resetPasswordRequest, resetPassword, onGoogleLoginSuccess, onFacebookLoginSuccess, updateEmailAndUsername, updatePassword, getUser, getAllUsers, blockUser, verifyEmail, resendVerificationEmail } = require("../controllers/userCtrl");
const { authorized, authorizeAdmin } = require("../middlewares/authorization");
const userModel = require("../models/userModel")

router.post("/register", registerUser);
router.post("/login", loginUser);
router.post("/updateUser", authorized, updateUser);
...
router.get('/checkLogin', authorized, async (req, res) => {
    ...
    if (!user) {
        return res.status(200).json({ success: false, isLogin: false });    
    }
